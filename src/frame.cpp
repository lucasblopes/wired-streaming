...
void socket_config(int sockfd, int timeout_seconds, int interface_index) {
    ...
    if (setsockopt(sockfd, SOL_PACKET, PACKET_ADD_MEMBERSHIP, &mr,
                   sizeof(mr)) == -1) {
        perror("setsockopt failed: Ensure the network interface is specified "
               "correctly");
        exit(EXIT_FAILURE);
    }
}

// Function to send a frame and wait for a response with its own timeout/ Função
// para enviar um
bool send_frame_with_timeout(int sockfd, Frame &frame, struct sockaddr_ll &addr,
                             int addrlen, int timeout_seconds) {
    auto start = chrono::steady_clock::now();
    char buffer[sizeof(Frame)] = {0};

    while (true) {
        // Send the frame
        sendto(sockfd, &frame, sizeof(Frame), 0, (struct sockaddr *)&addr,
               addrlen);
        cout << "Frame '" << frame.data << "' sent, waiting for response..."
             << endl;

        // Wait for response with timeout on socketsposta com timeout no socket
        ssize_t len = recvfrom(sockfd, buffer, sizeof(Frame), 0,
        ...
        if (len < 0) {
            ...
            if (response->start_marker == 0x7E &&
                ...
                if (crc == response->crc) {
                    cout << "Valid response received: " << response->data
                         << endl;
                    return true;
                } else {
...
bool receive_frame_with_timeout(int sockfd, struct sockaddr_ll &addr,
                                Frame &frame, int timeout_seconds) {
